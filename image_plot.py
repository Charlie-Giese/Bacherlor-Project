...
parser.add_argument("-b", "--boundaries", nargs="+", help="Boundaries of image in pixels", default=False)
...
def image_plot(inputfile, d_range, boundaries, outputfile):
...
def contour_plot(ax, contour_file, contour_type, image_beam):
	...
	if contour_type == "manual":
		...
		ax.contour(cdata, levels=levels, norm=norm, transform=ax.get_transform(cwcs.celestial), colors='black', alpha=0.5)




result=image_plot(inputfile, d_range, boundaries,  outputfile)
